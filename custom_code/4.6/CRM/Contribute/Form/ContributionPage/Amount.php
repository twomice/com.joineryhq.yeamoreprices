...
/*
 * ==========================================================================
 * This is an exact copy of CiviCRM's CRM/Contribute/Form/ContributionPage/Amount.php,
 * with the exception of changes marked with "MODIFIED.com.joineryhq.yeamoreprices"
 * See extension README.md for thougths on overriding PHP and templates.
 * ==========================================================================
 *
 +--------------------------------------------------------------------+
 | CiviCRM version 4.6                                                |
...
class CRM_Contribute_Form_ContributionPage_Amount extends CRM_Contribute_Form_ContributionPage {
   ...
   */
  /*
   * START MODIFIED.com.joineryhq.yeamoreprices:
   * Increase this to 16 instead of 11.
   */
  const NUM_OPTION = 16;
  /*
  ...
  public function buildQuickForm() {
    /*
     * START MODIFIED.com.joineryhq.yeamoreprices:
     * Pass this value to the template.
     */
    $this->assign('yeamoreprices_loop_limit', self::NUM_OPTION);
    /*
     * END MODIFIED.com.joineryhq.yeamoreprices:
     */
...
SELECT id
 ...
 WHERE id IN ({$paymentProcessorIds})
   AND is_recur = 1";
    ...
    $this->addWysiwyg('pay_later_receipt', E::ts('Pay Later Instructions'), CRM_Core_DAO::getAttribute('CRM_Contribute_DAO_ContributionPage', 'pay_later_receipt'));
    ...
    if (!empty($fields['is_recur_interval'])) {
      foreach (array_keys($fields['payment_processor']) as $paymentProcessorID) {
        ...
        if ($paymentProcessorType == 'Google_Checkout') {
          $errors['is_recur_interval'] = E::ts('Google Checkout does not support recurring intervals');
          break;
        }
      }
    }
